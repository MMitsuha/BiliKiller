...
namespace UEFI
{
	BootOption::BootOption(UINT8* buffer, const DWORD orderLen)
	{
		...
		wcsncpy_s(wstrBuffer.get(), orderLen, (wchar_t*)buffer, descLen);
		buffer += (descLen + 1) * 2;
	...
	bool BootOption::Pack(UINT8* buffer, size_t bufferLen)
	{
		...
		wcsncpy_s((wchar_t*)buffer, bufferLen / sizeof(wchar_t), m_Description.c_str(), m_Description.size());
	...
	std::wstring FilePath::ToMediaDevice() const
	{
		...
		wcsncpy_s(wstrBuffer.get(), m_Header.Length / 2, (wchar_t*)(m_Data.get() + sizeof(m_Header)), m_Header.Length / 2);
