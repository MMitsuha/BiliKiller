...
namespace UEFI
{
	...
	)
	{
		...
		wchar_t bootxxxx[9] = { 0 };
		swprintf_s(bootxxxx, 9, L"Boot%04x", bootIndex);
		...
		if (SetFirmwareEnvironmentVariableW(bootxxxx,
			UEFI_GUID,
			buffer.get(),
			bufferSize) == TRUE) {
			//        std::wcout << "[InsertBootOption] Success!! SetFirmwareEnvironmentVariableW" << std::endl;
		}
		else {
			std::wcout << "[InsertBootOption] Failed!! SetFirmwareEnvironmentVariableW" << std::endl;
		}
	...
	void RemoveBootOption(const wstring& bootxxxx)
	{
		...
		if (SetFirmwareEnvironmentVariableW(bootxxxx.c_str(),
			UEFI_GUID,
			NULL,
			NULL) == TRUE) {
			//        std::wcout << "Success!! SetFirmwareEnvironmentVariableW" << endl;
		}
		else {
			std::wcout << "Failed!! SetFirmwareEnvironmentVariableW" << GetLastError() << endl;
			return;
		}
		BootOrder btorder;
		UINT16 index = 0;
	...
	BootOption GetBootOption(const wstring& bootxxxx)
	{
		...
		DWORD orderLen = GetFirmwareEnvironmentVariableW(
			bootxxxx.c_str(), UEFI_GUID, buffer, bufSize);

		if (0 == orderLen) {
			// TODO:
			std::wcout << "Failed, GetFirmwareEnvironmentVariableW(), GetLastError return " << GetLastError();
		}
